...
const Tickets = () => {
    ...
    const handleCardClick = async (ticket_camera_id, ticket_id) => {
        try {
            const response = await fetch(`http://127.0.0.1:8000/cameras/${ticket_camera_id}`);
            if (!response.ok) throw new Error('Network response was not ok');
            const data = await response.json();
    ...
    const handleStatusChange = async (status, ticket_id) => {
        try {
            const response = await fetch(`http://127.0.0.1:8000/tickets/${ticket_id}?status=${status}`, {
                method: 'PUT',
                headers: {
                    ...
                    <div className="flex flex-col gap-6">
                        {tickets
                            ...
                            .map((ticket) => (
                                ...
                                >
                                    <CardHeader>
                                        <CardTitle className="text-lg">{ticket.description || "Untitled"}</CardTitle>
                                    </CardHeader>
                                    <CardContent>
